...
import xyz.hyperreal.dal.{PrecisionDAL, DAL, numberType}
...
object Interpolation {
  ...
  val INTERPOLATED_PATTERN: Regex =
...
class FunLParser extends StandardTokenParsers with PackratParsers {
  ...
  def parse[T](grammar: PackratParser[T], r: Reader[Char]): ParseResult[T] =
...
//		natives |
  ...
  lazy val definitions: Parser[DeclarationStatementAST] =
    "def" ~> definition |
      "def" ~> Indent ~> rep1(definition <~ Newline) <~ Dedent ^^ DeclarationBlockAST

  lazy val definition: Parser[DefAST] =
    pos ~ ident ~ opt("(" ~> rep1sep(structure, ",") ~ opt("...") <~ ")") ~ (optionallyGuardedPart | guardedParts) ^^ {
      case p ~ n ~ None ~ ((gs, w)) =>
        DefAST(n, FunctionExpressionAST(n, p, Nil, arb = false, gs, WhereClauseAST(w)))
  ...
  lazy val optionallyGuardedPart: PackratParser[(List[FunctionPartExpressionAST], List[DeclarationStatementAST])] =
    opt("|" ~> guardExpression) ~ ("=" ~> expressionOrBlock | blockExpression) ~ opt(whereClause | Indent ~> whereClause <~ Newline <~ Dedent) ^^ {
  ...
  lazy val guardedParts: PackratParser[(List[FunctionPartExpressionAST], List[DeclarationStatementAST])] =
    Indent ~> rep1(guardedPart) ~ opt(whereClause <~ Newline) <~ Dedent ^^ {
      case g ~ w => (g, w getOrElse Nil)
    }

  lazy val guardedPart: Parser[FunctionPartExpressionAST] =
  ...
  lazy val expressionOrBlock: Parser[ExpressionAST] = expression | blockExpression

  lazy val noAssignmentExpressionOrBlock: Parser[ExpressionAST] = compoundExpression1 | blockExpression

  lazy val blockExpression: Parser[BlockExpressionAST] = Indent ~> statements <~ Dedent ^^ BlockExpressionAST
  ...
  lazy val assignmentExpression: PackratParser[ExpressionAST] =
    rep1sep(pos ~ lvalueExpression, ",") ~ assignment ~ (rep1sep(pos ~ assignmentExpression, ",") | pos ~ blockExpression ^^ (List(_))) ^^ {
      case lhs ~ op ~ rhs =>
        AssignmentExpressionAST(lhs map { case p ~ e => (p, e) }, operator(op), rhs map {
          case p ~ e                                 => (p, e)
        })
    } |
  ...
  lazy val constructExpression: PackratParser[ExpressionAST] =
    "if" ~> expression ~ ("then" ~> expressionOrBlock | blockExpression) ~ rep(elif) ~ elsePart ^^ {
      ...
      opt(ident <~ ":") ~ ("every" ~> expression) ~ opt("do" ~> expressionOrBlock | blockExpression) ~ elsePart ^^ {
  ...
  lazy val elif: Parser[(ExpressionAST, ExpressionAST)] = onl ~> "elif" ~> expression ~ ("then" ~> expressionOrBlock | blockExpression) ^^ {
    case c ~ t => (c, t)
  }

  lazy val generator: Parser[GeneratorExpressionAST] = (structure <~ "<-") ~ pos ~ expression ~ opt("if" ~> logicalExpression) ^^ {
    case s ~ p ~ t ~ f => GeneratorExpressionAST(s, p, t, f)
  }

  lazy val generators: Parser[List[GeneratorExpressionAST]] = rep1sep(generator, ",")

  lazy val lvalueExpression: PackratParser[ExpressionAST] = generateDefinedExpression

  lazy val assignment: Parser[String] = "=" | "+=" | "++=" | "-=" | "--=" | "*=" | "/=" | "//=" | "\\=" | "^=" | "<:=" | ">:="
  ...
  lazy val partialFunctionExpression: Parser[PartialFunctionExpressionAST] =
  ...
  lazy val lambdaExpression: Parser[FunctionExpressionAST] =
    pos ~ parameters ~ opt("|" ~> guardExpression) ~ ("->" ~> opt(expressionOrBlock)) ^^ {
      case p ~ ((parms, a)) ~ g ~ b =>
        FunctionExpressionAST("$" + p.toString,
                              ...
                              parms,
                              a,
                              List(FunctionPartExpressionAST(g, b.getOrElse(LiteralExpressionAST(VMVoid)))),
                              WhereClauseAST(Nil))
    }
...
//  )
  ...
  lazy val controlExpression: PackratParser[ExpressionAST] =
    "fail" ^^^ FailExpressionAST |
      ...
      "return" ~> opt(expression) ^^ (e => ReturnExpressionAST(e.getOrElse(LiteralExpressionAST(VMVoid)))) |
  ...
  lazy val consExpression: PackratParser[ExpressionAST] =
    pos ~ rangeExpression ~ (":" ~> pos) ~ consExpression ^^ {
      case ph ~ h ~ pt ~ t =>
        BinaryExpressionAST(ph, h, Symbol(":"), pt, t)
    } |
      rangeExpression
  ...
  lazy val rangeExpression: PackratParser[ExpressionAST] =
    pos ~ (additiveExpression <~ "..") ~ pos ~ additiveExpression ~ opt("by" ~> pos ~ additiveExpression) ^^ {
      case pf ~ f ~ pt ~ t ~ Some(pb ~ b) =>
        RangeExpressionAST(pf, f, pt, t, pb, b, inclusive = true)
      case pf ~ f ~ pt ~ t ~ None =>
        RangeExpressionAST(pf, f, pt, t, null, LiteralExpressionAST(VM.ONE), inclusive = true)
    } |
      pos ~ (additiveExpression <~ "..<") ~ pos ~ additiveExpression ~ opt("by" ~> pos ~ additiveExpression) ^^ {
        case pf ~ f ~ pt ~ t ~ Some(pb ~ b) =>
          RangeExpressionAST(pf, f, pt, t, pb, b, inclusive = false)
        case pf ~ f ~ pt ~ t ~ None =>
          RangeExpressionAST(pf, f, pt, t, null, LiteralExpressionAST(VM.ONE), inclusive = false)
      } |
      pos ~ (additiveExpression <~ "..+") ~ pos ~ additiveExpression ~ opt("by" ~> pos ~ additiveExpression) ^^ {
        case pf ~ f ~ pt ~ t ~ Some(pb ~ b) =>
          RangeExpressionAST(pf, f, pt, BinaryExpressionAST(null, f, operator("+"), null, t), pb, b, inclusive = false)
        case pf ~ f ~ pt ~ t ~ None =>
          RangeExpressionAST(pf, f, pt, BinaryExpressionAST(null, f, operator("+"), null, t), null, LiteralExpressionAST(VM.ONE), inclusive = false)
      } |
      pos ~ (additiveExpression <~ "..-") ~ pos ~ additiveExpression ~ opt("by" ~> pos ~ additiveExpression) ^^ {
        case pf ~ f ~ pt ~ t ~ Some(pb ~ b) =>
          RangeExpressionAST(pf, f, pt, BinaryExpressionAST(null, f, operator("-"), null, t), pb, b, inclusive = false)
        case pf ~ f ~ pt ~ t ~ None =>
          RangeExpressionAST(pf,
                             f,
                             pt,
                             BinaryExpressionAST(null, f, operator("-"), null, t),
                             null,
                             LiteralExpressionAST(VM.MINUS_ONE),
                             inclusive = false)
      } |
      pos ~ (additiveExpression <~ "..") ~ opt("by" ~> pos ~ additiveExpression) ^^ {
        case pf ~ f ~ None =>
          UnboundedLazyListExpressionAST(pf, f, null, LiteralExpressionAST(VM.ONE))
        case pf ~ f ~ Some(pb ~ b) =>
      ...
      pos ~ additiveExpression ~ ("to" | "until") ~ pos ~ additiveExpression ~ opt("by" ~> pos ~ additiveExpression) ^^ {
        ...
        case pf ~ f ~ op ~ pt ~ t ~ None =>
          SequenceExpressionAST(pf, f, pt, t, null, LiteralExpressionAST(VM.ONE), if (op == "to") true else false)
      } |
      additiveExpression
  ...
  lazy val multiplicativeExpression: PackratParser[ExpressionAST] =
    pos ~ multiplicativeExpression ~ ("*" | "/" | """\""" | "%" | "\\%" | "//" | "mod" | "div" | "rotateright" | "rotateleft" | ">>>" | "<<") ~ pos ~ exponentialExpression ^^ {
      ...
      pos ~ multiplicativeExpression ~ pos ~ applyExpression ^^ {
        case pl ~ l ~ pr ~ r =>
          BinaryExpressionAST(pl, l, operator("*"), pr, r)
      } |
  ...
  lazy val unaryExpression: PackratParser[ExpressionAST] =
    "-" ~> pos ~ incrementExpression ^^ {
      case _ ~ LiteralExpressionAST(n: VMNumber) =>
        LiteralExpressionAST(PrecisionDAL.negate(n, VMNumber.apply))
  ...
  lazy val incrementExpression: PackratParser[ExpressionAST] =
    ("++" | "--") ~ pos ~ generateDefinedExpression ^^ {
      case o ~ p ~ e =>
        UnaryExpressionAST(Symbol(o + "*"), p, e)
    } |
      pos ~ generateDefinedExpression ~ ("++" | "--") ^^ {
        case p ~ e ~ o =>
          UnaryExpressionAST(Symbol("*" + o), p, e)
  ...
  lazy val applyExpression: PackratParser[ExpressionAST] =
    pos ~ applyExpression ~ pos ~ ("(" ~> repsep(pos ~ expression, ",") <~ ")") ^^ {
      case fp ~ f ~ ap ~ args =>
        ApplyExpressionAST(fp, f, ap, args map { case p ~ e => (p, e) }, tailrecursive = false)
    } |
  ...
  lazy val mapEntry: Parser[(ExpressionAST, ExpressionAST)] = keyExpression ~ (":" ~> expression) ^^ {
    case VariableExpressionAST(_, k, _) ~ v => LiteralExpressionAST(VMString(k)) -> v
    case k ~ v                              => (k, v)
  }

  lazy val keyExpression: PackratParser[ExpressionAST] = rangeExpression

  lazy val comprehension: PackratParser[ComprehensionAST] = (consExpression <~ "|") ~ generators ^^ {
  ...
  lazy val primaryExpression: PackratParser[ExpressionAST] =
    pos ~ regexLit ^^ {
      ...
      number ^^ (n => LiteralExpressionAST(VMNumber(n))) |
      pos ~ stringLit ^^ {
        case p ~ s =>
          if (s.nonEmpty && s.charAt(0) >= INTERPOLATION_DELIMITER) {
            ...
            for (m <- INTERPOLATED_PATTERN.findAllMatchIn(s))
              m.matched.charAt(0) match {
                case INTERPOLATION_LITERAL =>
                  buf.append(LiteralExpressionAST(VMString(m.group(1))))
            ...
            LiteralExpressionAST(VMString(s))
      } |
      "undefined" ^^^ LiteralExpressionAST(VMUndefined) |
      "(" ~> infix <~ ")" ^^ { o =>
        ...
        case p ~ e ~ o =>
          ...
          LeftSectionExpressionAST(
            ...
            FunctionExpressionAST(
              ...
              arb = false,
              List(FunctionPartExpressionAST(None, BinaryExpressionAST(null, e, s, p, VariableExpressionAST(null, "$", "$")))),
              WhereClauseAST(Nil)
            ),
            s
          )
        ...
        case o ~ p ~ e =>
          ...
          RightSectionExpressionAST(
            ...
            FunctionExpressionAST(
              ...
              List(VariableStructureAST(p, "$", "$")),
              arb = false,
              List(FunctionPartExpressionAST(None, BinaryExpressionAST(p, VariableExpressionAST(null, "$", "$"), s, null, e))),
              WhereClauseAST(Nil)
            )
          )
      } |
      ("true" | "false") ^^ (b => LiteralExpressionAST(VMBoolean(b.toBoolean))) |
      "(" ~ ")" ^^^ LiteralExpressionAST(VMVoid) |
      "null" ^^^ LiteralExpressionAST(VMNull) |
...
//      "{" ~ ":" ~ "}" ^^^ LiteralExpressionAST(Map()) |
  ...
  lazy val primaryStructure: PackratParser[StructureAST] =
    number ^^ (n => LiteralStructureAST(VMNumber(n))) |
      stringLit ^^ (s => LiteralStructureAST(VMString(s))) |
      "undefined" ^^^ LiteralStructureAST(VMUndefined) |
      "(" ~ ")" ^^^ LiteralStructureAST(VMVoid) |
      "null" ^^^ LiteralStructureAST(VMNull) |
