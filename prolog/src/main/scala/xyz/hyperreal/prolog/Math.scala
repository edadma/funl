...
object Math {
  ...
  class Function(obj: Any, method: Method) extends ((VM, IndexedSeq[CharReader]) => Unit) {
    def apply(vm: VM, pos: IndexedSeq[CharReader]) =
      vm.push(
        method
          .invoke(obj, (for (_ <- 1 to method.getParameterCount) yield vm.pop).reverse.toArray.asInstanceOf[Array[Object]]: _*)
          .asInstanceOf[Number])
