...
object Streams {
  ...
  def current_input(vm: VM, pos: IndexedSeq[CharReader], stream: Any) = vm.unify(stream, input)

  def current_output(vm: VM, pos: IndexedSeq[CharReader], stream: Any) = vm.unify(stream, output)

  def set_input(vm: VM, pos: IndexedSeq[CharReader], stream: Any) =
  ...
  def set_output(vm: VM, pos: IndexedSeq[CharReader], stream: Any) =
    Streams(stream) match {
  ...
  def open(vm: VM, pos: IndexedSeq[CharReader], file: Any, mode: Any, stream: Any, options: Any) =
  ...
  def close(vm: VM, pos: IndexedSeq[CharReader], stream: Any, options: Any) =
    stream match {
  ...
  def flush_output(vm: VM, pos: IndexedSeq[CharReader], stream: Any) =
    stream match {
