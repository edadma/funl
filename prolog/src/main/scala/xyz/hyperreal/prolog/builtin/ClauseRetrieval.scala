...
import xyz.hyperreal.prolog.{Indicator, Procedure, Structure, indicator}

object ClauseRetrieval {

  def clause(vm: VM, pos: IndexedSeq[CharReader], head: Any, body: Any) = {
    val p =
      head match {
  ...
  def current_predicate(vm: VM, pos: IndexedSeq[CharReader], pred: Any) =
