...
import xyz.hyperreal.prolog.{Structure, instantiationError, typeError}
...
object Flags {
  ...
  def get_flag(vm: VM, pos: IndexedSeq[CharReader], key: Any, value: Any) =
    key match {
  ...
  def set_flag(vm: VM, pos: IndexedSeq[CharReader], key: Any, value: Any) =
    (key, value) match {
  ...
  def flag(vm: VM, pos: IndexedSeq[CharReader], key: Any, old: Any, value: Any) =
    (key, value) match {
