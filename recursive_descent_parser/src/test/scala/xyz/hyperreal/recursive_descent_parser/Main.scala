...
object Main extends App {
  ...
  val cut = Action[(CharReader, String), AtomAST](Parser.symbol("!"), { case (pos, _) => AtomAST(pos, "!") })
  val anyAtom =
    Alternates(
      List(
    ...
    Alternates[AST](
      List(
        ...
        Sequence[AtomAST, List[AST], StructureAST](
          anyAtom,
          Parser.middle(Parser.symbol("("), Parser.oneOrMoreSeparated(rule900, Parser.symbol(",")), Parser.symbol(")")),
          (name, args) => StructureAST(name.pos, name.atom, args)
        ),
        anyNonSymbolAtom,
        SequenceLeft(
          Sequence[(CharReader, String), (List[AST], Option[AST]), ListAST](
            Parser.symbol("["),
            Sequence[List[AST], Option[AST], (List[AST], Option[AST])](Parser.oneOrMoreSeparated(rule900, Parser.symbol(",")),
                                                                       Optional(SequenceRight(Parser.symbol("|"), rule1200)),
                                                                       (_, _)), { case ((pos, _), (l, t)) => ListAST(pos, l, t) }
          ),
          Parser.symbol("]")
        ),
        Sequence[(CharReader, String), (CharReader, String), AtomAST](Parser.symbol("["), Parser.symbol("]"), (a, _) => AtomAST(a._1, "[]"))
      ))
  val (rules, ops) = Builder[AST](
    primary,
    List(
      Op(1200, XFX, ":-"),
      Op(1200, XFX, "-->"),
...
//  println( prettyPrint(ast) )
  ...
  clause(lexer.tokenStream(CharReader.fromFile("/home/ed/dev/prolog/examples/sudoku.prolog")))
